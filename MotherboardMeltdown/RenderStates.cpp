...
void RenderStates::InitAll(ID3D11Device* device)
{
	...
	mirrorDesc.DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ZERO; // wont write any thing ... not using the test 
	mirrorDesc.DepthFunc = D3D11_COMPARISON_LESS;
	mirrorDesc.StencilEnable = true;
	mirrorDesc.StencilReadMask = 0xff;
	mirrorDesc.StencilWriteMask = 0xff; //  hex value for all 1's in 8 bit ..11111111

	mirrorDesc.FrontFace.StencilFailOp = D3D11_STENCIL_OP_KEEP;
	mirrorDesc.FrontFace.StencilDepthFailOp = D3D11_STENCIL_OP_KEEP;
	mirrorDesc.FrontFace.StencilPassOp = D3D11_STENCIL_OP_REPLACE;
	mirrorDesc.FrontFace.StencilFunc = D3D11_COMPARISON_ALWAYS; // always going to pass the test and replace whats there 
	...
	//SHADOW STENCILBUFFER

	D3D11_DEPTH_STENCIL_DESC drawReflectionShadDesc;
	drawReflectionShadDesc.DepthEnable = true;
	drawReflectionShadDesc.DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ALL;
	drawReflectionShadDesc.DepthFunc = D3D11_COMPARISON_LESS;
	drawReflectionShadDesc.StencilEnable = true;
	drawReflectionShadDesc.StencilReadMask = 0xff;
	drawReflectionShadDesc.StencilWriteMask = 0xff;

	drawReflectionShadDesc.FrontFace.StencilFailOp = D3D11_STENCIL_OP_KEEP;
	drawReflectionShadDesc.FrontFace.StencilDepthFailOp = D3D11_STENCIL_OP_KEEP;
	drawReflectionShadDesc.FrontFace.StencilPassOp = D3D11_STENCIL_OP_INCR;
	drawReflectionShadDesc.FrontFace.StencilFunc = D3D11_COMPARISON_EQUAL;

	// We are not rendering backfacing polygons, so these settings do not matter.
	drawReflectionShadDesc.BackFace.StencilFailOp = D3D11_STENCIL_OP_KEEP;
	drawReflectionShadDesc.BackFace.StencilDepthFailOp = D3D11_STENCIL_OP_KEEP;
	drawReflectionShadDesc.BackFace.StencilPassOp = D3D11_STENCIL_OP_INCR;
	drawReflectionShadDesc.BackFace.StencilFunc = D3D11_COMPARISON_EQUAL;

	HR(device->CreateDepthStencilState(&drawReflectionShadDesc, &DrawReflectionDSSShadow));
	...
	noDoubleBlendDesc.FrontFace.StencilFailOp = D3D11_STENCIL_OP_KEEP; // test runs , fails and dont alter stencil buffer 
	noDoubleBlendDesc.FrontFace.StencilDepthFailOp = D3D11_STENCIL_OP_KEEP;
	noDoubleBlendDesc.FrontFace.StencilPassOp = D3D11_STENCIL_OP_INCR;
	noDoubleBlendDesc.FrontFace.StencilFunc = D3D11_COMPARISON_EQUAL; // Always causes garbage drawn
