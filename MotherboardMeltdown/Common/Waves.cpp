...
{
	float PolyMult = 4.0f;  //PolyGon Multiplier

	mNumRows = m *= PolyMult; // FOR MORE POLYS *= NUMBER 
	mNumCols = n *= PolyMult;
	...
	for(UINT i = 0; i < m; ++i)
	{
		float z = halfDepth / PolyMult - (i*dx) / PolyMult;			// FOR MORE POLYS halfDepth/NUMBER and (i*dx)/NUMBER
		for(UINT j = 0; j < n; ++j)
		{
			float x = -halfWidth / PolyMult + (j*dx) / PolyMult;		// FOR MORE POLYS halfDepth/NUMBER and (i*dx)/NUMBER
